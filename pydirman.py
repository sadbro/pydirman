...
def __display():
    ...
    print("YOUR CURRENT LOCATION: "+ colored("{}".format(CUR_DIR), "red", attrs=['bold']))
    print("===========================")
    print("[options] = [c]hoose/[e]xit/[p]revious/[s]earch ")
...
def __chdir(CUR_DIR):
    ...
    CUR_PATH = os.path.abspath(CUR_DIR)
    __all__ = ['choose', 'exit', 'previous', 'search']

    com = str(input(colored("\npydirman", "blue", attrs=['underline']) +colored(">", "blue")))
    if com.lower() == "c":
            ...
            try:
                if os.path.isdir(str(LS_DIR[ask])):
                    ...
                    if porr.lower() == "p":
                        ...
                        os.system("sudo gedit {}".format(CUR_FILE))
                        __chdir(os.getcwd())
